const apiKey = '7bb9b35cc693855543828b7927f507f0';		//open weather map apikey
const apiUrl =
	'https://api.openweathermap.org/data/2.5/weather?units=metric&q=';		//store url weater api

const searchBox = document.querySelector('.search input');      //dom element intearect to user interface 
...
async function checkWeather(city) {
	...
	if (response.status == 404) {
		...
		let colorGradient;
		switch (data.weather[0].main) {					//switch if weatehr rigt now
			case 'Clouds':
